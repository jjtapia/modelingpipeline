...
def compareTimeSeries(mdldataset):
    ...
    for timesampleIdx in range(len(timepoints)/params['segments'], len(timepoints), len(timepoints)/params['segments']):
        ...
        mdlslice = mdldataset.query('(Origin == "NFsim")').query('(Seconds == {0})'.format(timepoints[timesampleIdx]))
        ...
        for observable in keys:
            ...
            mdlobs = np.array(mdlslice.query('(Observable == "{0}")'.format(observable))['Value'])
            mdlrobs= np.array(mdlrslice.query('(Observable == "{0}")'.format(observable))['Value'])
            #print '0----'
            #print mdlobs
            #print '---'
            #print mdlrobs
            ...
            ksresult.append({'observable':observable,'time':timepoints[timesampleIdx],'p-value':pvalue,'D':kmetric})
    ...
    if error == 0:
        print bcolors.OKGREEN,'no issues were found!',bcolors.ENDC

    ksdb = pandas.DataFrame.from_dict(ksresult)
    print ksdb
    print np.mean(list(ksdb['p-value']))
...
if __name__ == "__main__":
    ...
    params['segments'] = 50
    databasename = namespace.test

    #generateData(params,databasename)

    mdldataset = pandas.read_hdf('{0}DB.h5'.format(databasename))
