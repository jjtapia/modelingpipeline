...
def getParamFiles(directory):
    ...
    for root, dirnames, filenames in os.walk(directory):
        for filename in fnmatch.filter(filenames, '*.bngl'):
...
if __name__ == "__main__":
	...
	parameterFiles =  getParamFiles('paramfiles')
	...
	for paramFile in parameterFiles:
		...
		with open('SpatialActionsIntermediate.bngl','w') as f:
			...
			f.write('simulate_ode({{suffix=>"{0}",t_start=>0,t_end=>400,n_steps=>4000}})\n'.format(paramFile.split('.')[0].split('/')[1]))
			f.write('writeSBML({{suffix=>"{0}"}})\n'.format(paramFile.split('.')[0].split('/')[1]))
		#run bionetgen
		call(['perl',bngDistro,'Motivational_example_cbngl_fixedparameterswoParam.bngl'])

		modifyReleaseNumber('Motivational_example_cbngl_fixedparameterswoParam_{0}.xml'.format(paramFile.split('.')[0].split('/')[1]))
